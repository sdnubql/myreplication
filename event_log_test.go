...
func TestBinlogWriteEvent(t *testing.T) {
	mockPack := []byte{
		0x82, 0x00, 0x00, // pack length
		0x06,                   // pack sequence
		0x00,                   // ok byte
		0x13, 0x0a, 0x1d, 0x55, // timestamp
		0x17,                   // event type
		0x01, 0x00, 0x00, 0x00, // server id
		0x81, 0x00, 0x00, 0x00, // event size 129
		0xf3, 0x01, 0x00, 0x00, // log pos
		0x00, 0x00, // flags
		0x26, 0x00, 0x00, 0x00, 0x00, 0x00, 0x07, 0x00,
	...
	reader := newPackReader(bytes.NewBuffer(mockPack))
	pack, err := reader.readNextPack()
	if err != nil {
		t.Error("Got error", err)
	}

	if pack.sequence != 0x06 {
		t.Error("pack sequence not match 0x06")
	}

	header := &eventLogHeader{}
	header.readHead(pack)
	if header.EventType != 0x17 {
		t.Error("event type not match 0x17")
	}
